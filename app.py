...
# ---------- Custom CSS for Minimal Look with Light Blue Background ----------
st.markdown(
    """
    <style>
...
def recursive_forecast(days_ahead, base_row):
    ...
    for day in range(days_ahead):
        ...
        forecast_date = pd.to_datetime(current_features["date"].iloc[0]) + timedelta(days=1)
        ...
        for feature in ["temperature_2m_mean", "precipitation_sum", "weather_code"]:
            ...
            if feature == "temperature_2m_mean":
                current_features[f"{feature}_lag1"] = temp_pred
            elif feature == "precipitation_sum":
                current_features[f"{feature}_lag1"] = precip_pred
            else:
                current_features[f"{feature}_lag1"] = code_pred
